...
MPlayer::MPlayer(const std::string& _filename) : m_strName(_filename), m_pPhysics(nullptr),
...
void MPlayer::Update(float _delta)
{
...
void MPlayer::GetLadderRope(std::list<Maple::LADDER_ROPE>& _ladderrope)
{
	m_pLdRope = &_ladderrope;
}

Gdiplus::Rect const& MPlayer::GetColRc()
{
...
void MPlayer::HitDamage(int _damage)
{
	m_bHit = true;
	m_bCollision = false;
	m_dwHitTick = 0;
...
void MPlayer::SetLevitation()
{
	m_pPhysics->SetLevitation();
}

bool MPlayer::IsCollision()
{
	return m_bCollision;
}

void MPlayer::HandleInput(EMAnimType _atype)
{
...
void MPlayer::SetComponent(Component* _pComp)
{
	if (_pComp != nullptr)
...
void MPlayer::LoadData(const std::string& _filename)
{
